...
class HPPActionServer(Node):
    ...
    def _cancel_cb(self, goal_handle):
        ...
        self.get_logger().info(f"{goal_handle.task}")
    ...
    def _execute_cb(self, goal_handle) -> PlanBarGrasp.Result:
        ...
        try:
            match task:
                case "pick":
                    success, message, path_id, traj = self._plan_pick(
                        gripper_name, handle_name
                    )
                case "place":
                    success, message, path_id, traj = self._plan_place(
                        gripper_name, handle_name
                    )
                case _:
                    message = f"_execute_ plan : task '{task}' not in ['pick','place']"
    ...
    def _plan_pick(self, gripper, handle):
        ...
        if traj is None:
            return False, "Pick planning failed.", -1, None

        self._q_after_grasp = q_end
        path_id = self._hpp._ps.numberPaths() - 1
        self.get_logger().info(f"¨Pick planned successfully (path_id={path_id})")
        return True, "Pick planned successfully.", path_id, traj
    ...
    def _plan_place(self, gripper, handle):
        ...
        if traj is None:
            return False, "Place planning failed.", -1, None

        path_id = (
            self._hpp._ps.numberPaths() - 1
        )  # TODO : see if we keep transmitting the path id
    ...
    def _build_q_init(self):
        ...
        for i, joint_name in enumerate(self._joint_state.name):  # TODO duplicated code
    ...
    def _convert_point(
        ...
        for joint_name in self._robot_model.names.tolist():
            full = f"tiago_pro/{joint_name}"
            if full in self._joints_config["actuated_joints"]:  # robot.jointNames:
                rank = robot.rankInConfiguration[full]
                value = q_env[rank]
                nq = robot.getJointConfigSize(full)
                if nq == 2:
